...
describe("Catalog Routes", () => {
    describe("POST /products", () => {
      ...
      test("should response with validation error 400", async () => {
        const requestBody = mockRequest();
        const response = await request(app).post("/products").send({ ...requestBody, name: "" }).set("Accept", "application/json");
    ...
    describe("PATCH /products/:id", () => {
      test("should update product successfully", async () => {
        ...
        const response = await request(app).patch(`/products/${product.id}`).send(requestBody).set("Accept", "application/json");
    ...
    describe("DELETE /products/:id", () => {
      test("should delete a product by id", async () => {
        ...
        const response = await request(app).delete(`/products/${product.id}`).set("Accept", "application/json");
        expect(response.status).toBe(200);
